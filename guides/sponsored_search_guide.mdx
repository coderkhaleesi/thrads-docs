...
# Sponsored Search Integration Guide
...
# Usage examples:
# User clicked on sponsored search
track_search_interaction(token, clicked=True)

# User performed a search (whether they clicked or not)
track_search_interaction(token, clicked=False, user_search="minimalist shelves for living room")

# User clicked and then searched
track_search_interaction(token, clicked=True, user_search="minimalist shelves")
```
